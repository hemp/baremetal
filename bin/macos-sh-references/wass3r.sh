...
# reboot
# -
# csrutil check; if enabled, then prompt
if [[ $(csrutil status) != *disabled* ]]; then
    echo "You should really turn off SIP before proceeding."
    exit 0;
fi
...
# Add Brew taps
curl https://gist.githubusercontent.com/raw/61d113b6ef5c48c5c48f | tr '\n' '\0' | xargs -0 -n1 brew tap

# Add Casks
curl https://gist.githubusercontent.com/raw/2842750084ec33e341a4 | tr '\n' '\0' | xargs -0 -n1 brew cask install

# Install from brew
curl https://gist.githubusercontent.com/raw/31f788bb9d3af3b9dabb | tr '\n' '\0' | xargs -0 -n1 brew install

# Wait for downloads/installs to finish
wait
...
# Set highlight color to blue-ish
defaults write NSGlobalDomain AppleHighlightColor -string "0.341176 0.568627 0.756862"
...
# Setup clock (24-hour format with date/day of week)
...
# Hide Spotlight tray-icon (and subsequent helper); requires disabling Apple SIP protection
